...
sort(unique(spei.dat$year))

## read in spei data
spei.dat <- read.csv("data/not_used_currently/SPEI_all_EDGE.csv") %>%
  mutate(experiment.years = ifelse(year %in% c(2012:2021), "Y", "N"), 
         site = ifelse(Site == "HAYS", "HYS", ## change site names to match cover data
                       ifelse(Site == "SEV_BLACK", "SBK", 
                              ifelse(Site == "SEV_BLUE", "SBL", Site)))) %>%
  select(-Site) ## get rid of capitalized column, keep uncapitalized to match cover data

# Clean ####
## classify SPEI years 
spei.filt <- spei.dat %>%
  filter(ifelse(site == "SBK" | site == "SBL", month == 10, month == 9)) %>% ## filter to growing season spei; spei values should be the 6 months preceding the month value
  ...
  mutate(exp.year = ifelse(year %in% c(2013:2019) & site %in% c("SBL", "SBK"), 
                           "Exp Drought",
                           ifelse(year %in% c(2014:2017) & site %in% 
                                    c("KNZ", "HYS", "CHY", "SGS"), 
                                  "Exp Drought", 
                                  ifelse(year %in% c(2020:2023) & site %in% 
                                           c("SBL", "SBK"), 
                                         "Exp Post-Drought",
...
ggplot(spei.filt) +
  geom_density(aes(x=spei)) +
  facet_wrap(~site) +
  geom_jitter(data = spei.exp, aes(x=spei, y=0.1, color = exp.year),
              height = 0.05, width = 0) +
