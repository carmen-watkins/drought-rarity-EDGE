...
SR_drought = ggplot(edge_RR, aes(x= spatial_rarity, y=resp.ratio.site_D4, 
  ...
  annotate("text", x = 0.1, y=1.16, label = "R[m]^2: 0.14", size = 3.5, parse = TRUE) +
  annotate("text", x = 0.4, y=1.16, label = "R[c]^2: 0.21", size = 3.5, parse = TRUE) +
  annotate("text", x = 0.25, y=1.38, label = "Slope: 0.87 [0.67, 1.08]", size = 3.5, parse = FALSE)
...
SR_postdrought = ggplot(edge_RR, aes(x=spatial_rarity, y=resp.ratio.site_PDfull, 
  ...
  theme(text = element_text(size = 13)) +
  coord_cartesian(ylim = c(-1,1.38)) +
  annotate("text", x = 0.1, y=1.16, label = "R[m]^2: 0.11", size = 3.5, parse = TRUE) +
  annotate("text", x = 0.4, y=1.16, label = "R[c]^2: 0.14", size = 3.5, parse = TRUE) +
  annotate("text", x = 0.25, y=1.38, label = "Slope: 0.76 [0.56, 0.97]", size = 3.5, parse = FALSE)
...
TR_drought = ggplot(edge_RR, aes(x=temporal_rarity, y=resp.ratio.site_D4, 
  ...
  annotate("text", x = 0.25, y=1.38, label = "Slope: 0.72 [0.55, 0.89]", size = 3.5, parse = FALSE)
...
TR_postdrought = ggplot(edge_RR, aes(x=temporal_rarity, y=resp.ratio.site_PDfull, 
  ...
  annotate("text", x = 0.25, y=1.38, label = "Slope: 0.57 [0.40, 0.74]", size = 3.5, parse = FALSE)
