...
## try another way for contrast
visreg(md4t,"temporal_rarity", by="BP.dom.site", gg = TRUE, breaks = 6) +
  xlab("Temporal Rarity") +
  ylab("Fit of Drought Response Ratio")

ggsave("figures/Dec2024/drr-raritydom-int_3panel.png", width = 8, height = 3.25)


visreg(md4t,"temporal_rarity", by="z_precip", gg = TRUE, breaks = 6) +
  xlab("Temporal Rarity") +
  ylab("Fit of Drought Response Ratio")
...
# Post-Drought ####
## Rarity-Ppt Int ####
tpd2 = visreg(mpdt_full,"temporal_rarity", by="z_precip",
             overlay = TRUE, partial = FALSE, rug = FALSE,
             plot=FALSE)

ggplot(tpd2$fit, aes(temporal_rarity, visregFit, linetype=factor(z_precip), fill=factor(z_precip))) +
  geom_ribbon(aes(ymin=visregLwr, ymax=visregUpr), alpha=0.25, 
              colour="grey50", linetype=1, size=0.2) +
  geom_line(linewidth = 2, aes(color = factor(z_precip)), linetype = "solid") +
  scale_color_manual(values = c("#F8AFA8", "#FDDDA0", "#74A089")) +
  ...
  labs(linetype="Site Precipitation", fill="Site Precipitation", color = "Site Precipitation") +
  xlab("Temporal Rarity") +
  ylab("Fit of Post-Drought Response Ratio")

ggsave("figures/Dec2024/pdrr-temprarityppt-int.png", width = 6, height = 3.75)



test=visreg(mpdt_full,"temporal_rarity", by="BP.dom.site",
       overlay = TRUE, partial = FALSE, rug = FALSE,
       plot=FALSE)

ggplot(test$fit, aes(temporal_rarity, visregFit, linetype=factor(BP.dom.site), fill=factor(BP.dom.site))) +
  geom_ribbon(aes(ymin=visregLwr, ymax=visregUpr), alpha=0.25, 
              colour="grey50", linetype=1, size=0.2) +
  geom_line(linewidth = 2, aes(color = factor(BP.dom.site)), linetype = "solid") +
  scale_color_manual(values = c("#e597b9", "#ad5fad", "#573b88")) +
...
ggsave("figures/Dec2024/pdrr-temprarity-sitedom-int.png", width = 6, height = 3.75)
