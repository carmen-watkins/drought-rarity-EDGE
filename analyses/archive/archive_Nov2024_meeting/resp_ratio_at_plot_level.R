...
drought.SE.RII <- edge_all %>%
  ...
  group_by(site, treatment, block, species) %>%
...
ggplot(drought.SE.RII, aes(x = resp.ratio.site, y = SE.RII)) +
  geom_point()

sum_RII = drought.SE.RII %>%
  group_by(site, species) %>%
  summarise(meanRR = mean(resp.ratio.site), 
            meanSERR = mean(SE.RII, na.rm = TRUE), 
            num.obs = n())

ggplot(sum_RII, aes(x = meanRR, y = meanSERR)) +
  geom_point()
